...
public class RecicladoraController {
    ...
    public String activar(Integer idRecicladora) throws Exception {
        Recicladora r = recicladoraDAO.getByIdRP(idRecicladora);
        if (r == null) {
            throw new Exception("Recicladora no encontrada");
        }
        if (r.getActiva()) {
            throw new Exception("La recicladora ya está activa");
        }

        recicladoraDAO.reactivateRP(idRecicladora);
        return "Recicladora activada correctamente";
    }
}
